/* "Fast" implementation of asc2un.c with all combinations of a Large Buffer (LB),
	...
	This has the added feature that the file operations are not
	buffered in user space, which may be faster.
	The idea is to have the fastest possible file access implementation. */
		...
		Behavior is modeled after CopyFile. */
{
	...
	hIn = CreateFile (fIn, GENERIC_READ, 0, NULL, OPEN_EXISTING, 
			FILE_ATTRIBUTE_NORMAL | SSF | FILE_FLAG_NO_BUFFERING, NULL);
	...
	while (ReadFile (hIn, aBuffer, BUF_SIZE, &nIn, NULL) && nIn > 0 && WriteOK) {
		...
		/* You need to write the complete output buffer due to the
			no buffering option */
		WriteOK = WriteFile (hOut, uBuffer, sizeof(uBuffer), &nOut, NULL);
	}

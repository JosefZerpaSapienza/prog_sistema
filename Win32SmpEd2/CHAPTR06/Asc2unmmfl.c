/* Chapter 6.
	asc2unMMfl.c function: Memory Mapped, with flushing, implementation
	of the ASCII to UNICODE file copy function. */
...
*/
	...
	if (!FlushViewOfFile (pOutFile, 0))
		ReportException (_T("Failure flushing view of output file."), 9);
	...
	if (!FlushFileBuffers (hOut)) 
		ReportException (_T ("Failure flushing output file."), 10);
	CloseHandle (hOut);
	Complete = TRUE;
	return TRUE;
}

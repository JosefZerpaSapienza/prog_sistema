...
/* The job database is maintained in a shared file in the C:\temp directory.
	...
	is large, the code will fail in several places.
	Return -1 in case of failure. */
{
	...
	/*  Lock the entire file plus one record for exclusive
		access as we will be making a new entry. */
	...
	_try { /* Assure that the region is unlocked. */
	...
	/* Either an empty slot has been found, or we are at end
		of the file and need to create a new one. */
	...
	_finally {		/* Release the lock on the file. */

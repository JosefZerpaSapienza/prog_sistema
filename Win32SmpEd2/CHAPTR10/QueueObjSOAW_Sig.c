/* Chapter 10. QueueObjSOAW_Sig.c	SingalObjectAndWait version		*/
/* Queue functions												*/
...
#define _WIN32_WINNT 0x400  /* See WINBASE.H. NT 4.0 and later required */
#include "EvryThng.h"
#include "SynchObj.h"
...
DWORD q_get (queue_t *q, PVOID msg, DWORD msize, DWORD MaxWait)
{
	...
	while (q_empty (q) && !TimedOut) {
		TimedOut = 
			(SignalObjectAndWait (q->q_guard, q->q_ne, MaxWait, FALSE) == WAIT_TIMEOUT);
...
DWORD q_put (queue_t *q, PVOID msg, DWORD msize, DWORD MaxWait)
{
	...
	while (q_full (q) && !TimedOut) {
		TimedOut = 
			(SignalObjectAndWait (q->q_guard, q->q_nf, MaxWait, FALSE) == WAIT_TIMEOUT);
//		ReleaseMutex (q->q_guard);
...
DWORD q_initialize (queue_t *q, DWORD msize, DWORD nmsgs)
{
	...
	q->q_ne = CreateEvent (NULL, FALSE, FALSE, NULL);  /* Auto-reset events */

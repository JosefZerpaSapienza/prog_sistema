...
_declspec (dllimport) BOOL ReceiveCSMessage (REQUEST *pRequest, SOCKET);
_declspec (dllimport) BOOL SendCSMessage (RESPONSE *pResponse, SOCKET);
...
int _tmain (DWORD argc, LPCTSTR argv [])
{
	...
	__tprintf (_T("All server resources deallocated. ExitProcess(0)\n");
	ExitProcess(0);
	return 0;
}
...
/* Server thread function. There is a thread for every potential client. */
{
	...
	while (!Done && !ShutFlag) { 	/* Main Command Loop. */
		...
		Disconnect = ReceiveCSMessage (&Request, ConnectSock);
		...
		if (fp != NULL) {
			...
			while ((fgets (Response.Record, MAX_RQRS_LEN, fp) != NULL)) {
				SendCSMessage (&Response, ConnectSock);
			}
		}
